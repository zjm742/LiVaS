...
setproctitle.setproctitle("zjm-no-kill")
...
#        print("train acc:", acc_avg.avg, "id acc:", id_acc_avg.avg)
        ...
        dev_acc, dev_f1,dev_inf1 = evaluate(args, model, dev_dataloader, tag="dev")
        # print("dev result:", dev_res)
        print("dev result acc and f1 scroe:", dev_acc, dev_f1,dev_inf1)
        print("-" * 15)
        sys.stdout.flush()
        # print(results)
        ...
        if dev_f1 > best_eval:
            ...
            test_acc, f1_score, id_f1, ood_f1, labels_all, preds_all,attention_all= evaluate(args, model, test_dataloader, tag="test")
            print("test result acc and f1 scroe:", test_acc, f1_score)
            # i += 1
            # print("Saving test results to labels_and_predictions{}.csv".format(i))

            # ood_res = detect_ood()
            best_eval = dev_f1
            # final_res = dict(ood_res, **{"test_acc": results['test_accuracy'], 'eval_acc': best_eval})
            final_res = dict(
                 {"test_acc": test_acc, 'eval_f1': best_eval, "f1": f1_score, "eval_acc": dev_acc, "in_f1": id_f1,"ood_f1":ood_f1})
            patient = 0
        else:
            patient += 1

        if (patient > 20 and epoch > 50) or epoch == args.num_train_epochs:
            df = pd.DataFrame({'True Label': labels_all.flatten(),
                             'Predicted Label': preds_all.flatten()
                             })
            df.to_csv('labels_and_predictions_our.csv', index=False)
            # np.save('attention_all_our.npy', attention_all)
...
def evaluate(args, model, eval_dataset, id_dataset=None, tag="dev"):
    ...
    if tag == "test":
        ...
        with torch.no_grad():
            ...
            return acc_avg.avg, np.mean(f1_scores), np.mean(f1_scores[:-1]),f1_scores[-1],np.concatenate(labels_all, axis=0),np.concatenate(preds_all, axis=0),attention_all
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default="bert", type=str,
                        help="roberta-large;bert-base-uncased")
    parser.add_argument("--max_seq_length", default=128, type=int)
    parser.add_argument("--task_name", default="clinc150", type=str)
    parser.add_argument("--data_dir", default="data/banking", type=str)
    ...
    parser.add_argument("--save_results_path", type=str, default='few-shot-nlp-bank',
